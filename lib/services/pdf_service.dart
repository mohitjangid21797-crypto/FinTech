...
class PDFService {
  static Future<File> generateLoanCompletionPDF(
          ...
          return [
            pw.Column(
              ...
              children: [
                ...
                pw.SizedBox(height: 10),
                ..._buildEMIWidgets(emis),
                pw.SizedBox(height: 20),

                // Completion Message - Conditional based on pending EMIs
                pw.Center(
                  child: pw.Text(
                    emis.any((emi) => !emi.isPaid)
                        ? 'Pay All Pending EMI'
    ...
    // Save PDF to loan_pdfs directory with correct naming convention
    Directory output;
    try {
    ...
    final pdfDir = Directory(path.join(output.path, 'loan_pdfs'));
    await pdfDir.create(recursive: true);

    final file = File(path.join(pdfDir.path, 'loan_${loan.id}_completion.pdf'));
  ...
  static List<pw.Widget> _buildEMIWidgets(List<EMISchedule> emis) {
    List<pw.Widget> widgets = [];
    if (emis.length <= 10) {
      widgets.add(_buildEMITable(emis));
    } else {
      // First page: 10 EMIs
      widgets.add(_buildEMITable(emis.sublist(0, 10)));
      widgets.add(pw.NewPage());
      // Subsequent pages: 20 EMIs each
      for (int i = 10; i < emis.length; i += 20) {
        int end = (i + 20 < emis.length) ? i + 20 : emis.length;
        widgets.add(_buildEMITable(emis.sublist(i, end)));
        if (end < emis.length) {
          widgets.add(pw.NewPage());
        }
      }
  ...
  /// Saves PDF to Downloads folder (Android) or shares (iOS).
  /// Pure service - returns success bool only.
  static Future<bool> savePDFToDownloads(File pdfFile) async {
    try {
      if (Platform.isAndroid) {
        ...
        try {
          ...
          debugPrint('Downloads dir not available: $e');
        }
        ...
        if (downloadsDir != null) {
          ...
          final filename = path.basename(pdfFile.path);
          final savedFile = File(path.join(appDir.path, 'Downloads', filename));
          await savedFile.parent.create(recursive: true);
          await pdfFile.copy(savedFile.path);
          await OpenFile.open(savedFile.path);
          debugPrint('Saved to app Documents/Downloads: ${savedFile.path}');
          return true;
        }
      } else {
        // iOS/Mac: Share
        await Share.shareXFiles([
          XFile(pdfFile.path),
        ], text: 'Loan PDF - Save to Files app');
        return true;
      }
    } catch (e) {
  ...
  static pw.Widget _buildEMITable(List<EMISchedule> chunk) {
    return pw.Table(
      border: pw.TableBorder.all(color: PDFTheme.primaryBlue, width: 1),
      columnWidths: {
      ...
      children: [
        ...
        ...chunk.map(
          (emi) => pw.TableRow(
            decoration: pw.BoxDecoration(color: PDFTheme.surfaceGrey),
            children: [
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text(emi.dueDate.toString().split(' ')[0]),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('Rs. ${emi.amount.toStringAsFixed(2)}'),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text(
                  ...
                  style: pw.TextStyle(
                    color: emi.isPaid
                        ...
                        : PdfColor.fromInt(0xFFE53935), // Bold red for Pending
                    fontWeight: pw.FontWeight.bold,
                  ),
                ),
              ),
