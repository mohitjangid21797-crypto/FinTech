...
class _AllPDFScreenState extends State<AllPDFScreen>
  ...
  final Map<File, bool> _deletingItems = {}; // Track per-item delete state
  Key? _futureKey;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(l10n.allLoanPDFs, style: theme.appBarTheme.titleTextStyle),
        elevation: 0,
              ...
              return Center(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(l10n.loadingPDFs, style: theme.textTheme.bodyLarge),
                  ],
                ),
              );
            }
  ...
  Future<List<_PdfItem>> _getPDFItems() async {
    try {
      ...
      final files = pdfDir
          .listSync()
          .where(
            (f) => f is File && path.extension(f.path).toLowerCase() == '.pdf',
          )
          .cast<File>()
          .where(
            (f) =>
    ...
    return Card(
      ...
      child: ExpansionTile(
        leading: Container(
          ...
          child: Icon(Icons.folder, color: Colors.red, size: 28),
        ),
        title: Text(
          group.customerName,
  ...
  Widget _buildPDFCard(_PdfItem item) {
    ...
    final isDeleting = _deletingItems[item.file] ?? false;
      ...
      child: ListTile(
        ...
        subtitle: customer != null
                ...
                children: [
                  Text(
                    ...
                    style: theme.textTheme.titleSmall?.copyWith(
                      ...
                      color: isDeleting ? Colors.grey : AppTheme.textPrimary,
                    ),
                  ),
                  Text(
                    customer.mobile,
                    style: theme.textTheme.bodySmall?.copyWith(
                      color: isDeleting ? Colors.grey : AppTheme.textSecondary,
                    ),
                  ),
                  Text(
            ...
            : Text(
                ...
                style: theme.textTheme.bodySmall?.copyWith(
                  color: isDeleting ? Colors.grey : Colors.grey,
                ),
              ),
        trailing: isDeleting
            ? null
        ...
        onTap: isDeleting ? null : () => _sharePDF(item.file, mobile),
  ...
  void _deletePDF(File file) async {
    ...
    // Mark as deleting
    setState(() {
      _deletingItems[file] = true;
    });

    try {
      // Direct delete - NO PERMISSIONS NEEDED for app documents directory
      await file.delete();

      // Remove from local deleting state
      if (mounted) {
        setState(() {
          _deletingItems.remove(file);
        });
